import type {
  ...
  UserAuthOptions,
  RequestOptions,
} from '@commercetools/sdk-client-v2/src/types/sdk'
...
export default class SdkAuth {
  ...
  _process(request: AuthRequest): Promise<TokenInfo> {
  ...
  static _parseResponseJson(response: any): Object | any {
    return response.json().catch(() => ({ statusCode: response.status }))
  }

  static _isErrorResponse(response: Object | any): boolean {
  ...
  static _enrichTokenResponse(response: Object | any): Promise<TokenInfo> {
  ...
  static _handleResponse(uri: string, response: Response): Promise<TokenInfo> {
  ...
  static _enrichUriWithProjectKey(
    uri: string,
  ...
  refreshTokenFlow(
    token: string,
    config: CustomAuthOptions = {}
  ): Promise<TokenInfo> | Promise<Object> {
  ...
  customFlow(requestConfig: RequestOptions): Promise<Object> {
